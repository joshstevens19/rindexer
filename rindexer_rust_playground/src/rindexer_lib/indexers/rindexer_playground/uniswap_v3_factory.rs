...
use super::super::super::typings::rindexer_playground::events::uniswap_v3_factory::{
    PoolCreatedEvent, UniswapV3FactoryEventType, no_extensions,
};
...
EthereumSqlTypeWrapper::U256(result.tx_information.log_index)
                    ...
                    if !csv_bulk_data.is_empty() {
                        ...
                        if let Err(e) = csv_result {
                            rindexer_error!("UniswapV3FactoryEventType::PoolCreated inserting csv data: {:?}", e);
                    ...
                    if postgres_bulk_data.len() > 100 {
                        let result = context
                            ...
                            .bulk_insert_via_copy(
                                "rindexer_playground_uniswap_v3_factory.pool_created",
                                &rows,
                                &postgres_bulk_data
                                    .first()
                        ...
                        if let Err(e) = result {
                            rindexer_error!("UniswapV3FactoryEventType::PoolCreated inserting bulk data via COPY: {:?}", e);
                            return Err(e.to_string());
                        }
                        } else {
                            let result = context
                            ...
                            if let Err(e) = result {
                                rindexer_error!("UniswapV3FactoryEventType::PoolCreated inserting bulk data via INSERT: {:?}", e);
                                return Err(e.to_string());
                            }
                    }
                                ...
                                rindexer_info!(
                                    "UniswapV3Factory::PoolCreated - {} - {} events",
                                    "INDEXED".green(),
                                    results.len(),
                                );
    ...
    UniswapV3FactoryEventType::PoolCreated(handler).register(manifest_path, registry).await;
}
