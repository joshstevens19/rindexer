...
async fn regular_width_signed_integers_handler(
    ...
    let handler = RegularWidthSignedIntegersEvent::handler(|results, context| async move {
                                if results.is_empty() {
                                    return Ok(());
                                }
                    ...
                    let mut csv_bulk_data: Vec<Vec<String>> = vec![];
                    for result in results.iter() {
                        csv_bulk_data.push(vec![result.tx_information.address.to_string(),result.event_data.i8.to_string(),
...
async fn regular_width_unsigned_integers_handler(
    ...
    let handler = RegularWidthUnsignedIntegersEvent::handler(|results, context| async move {
                                if results.is_empty() {
                                    return Ok(());
                                }
                    ...
                    let mut csv_bulk_data: Vec<Vec<String>> = vec![];
                    for result in results.iter() {
                        csv_bulk_data.push(vec![result.tx_information.address.to_string(),result.event_data.u8.to_string(),
...
async fn irregular_width_signed_integers_handler(
    ...
    let handler = IrregularWidthSignedIntegersEvent::handler(|results, context| async move {
                                if results.is_empty() {
                                    return Ok(());
                                }
                    ...
                    let mut csv_bulk_data: Vec<Vec<String>> = vec![];
                    for result in results.iter() {
                        csv_bulk_data.push(vec![result.tx_information.address.to_string(),result.event_data.i24.to_string(),
...
async fn irregular_width_unsigned_integers_handler(
    ...
    let handler = IrregularWidthUnsignedIntegersEvent::handler(|results, context| async move {
                                if results.is_empty() {
                                    return Ok(());
                                }
                    ...
                    let mut csv_bulk_data: Vec<Vec<String>> = vec![];
                    for result in results.iter() {
                        csv_bulk_data.push(vec![result.tx_information.address.to_string(),result.event_data.u24.to_string(),
...
async fn under__score_handler(manifest_path: &PathBuf, registry: &mut EventCallbackRegistry) {
    let handler = Under_ScoreEvent::handler(|results, context| async move {
                                if results.is_empty() {
                                    return Ok(());
                                }
                    ...
                    let mut csv_bulk_data: Vec<Vec<String>> = vec![];
                    for result in results.iter() {
                        csv_bulk_data.push(vec![result.tx_information.address.to_string(),result.event_data.foo.to_string(),
result.tx_information.transaction_hash.to_string(),result.tx_information.block_number.to_string(),result.tx_information.block_hash.to_string(),result.tx_information.network.to_string(),result.tx_information.transaction_index.to_string(),result.tx_information.log_index.to_string()]);
                        let data = vec![
EthereumSqlTypeWrapper::Address(result.tx_information.address),
...
async fn capitalized_handler(manifest_path: &PathBuf, registry: &mut EventCallbackRegistry) {
    let handler = CAPITALIZEDEvent::handler(|results, context| async move {
                                if results.is_empty() {
                                    return Ok(());
                                }
