...
pub fn rindexer_yaml_exists(project_path: &PathBuf) -> bool {
    fs::metadata(project_path.join(YAML_CONFIG_NAME)).is_ok()
}

pub fn rindexer_yaml_does_not_exist(project_path: &PathBuf) -> bool {
    !rindexer_yaml_exists(project_path)
}

pub fn validate_rindexer_yaml_exist(project_path: &PathBuf) {
