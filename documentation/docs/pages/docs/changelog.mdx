...
- fix: **Checkpoint rewinds during reorg rollback now use `LEAST()`** — a rollback can never move a still-backfilling event's `last_synced_block` checkpoint forward. Affects all projects, not just `sync_together`.
- fix: **Graceful shutdown no longer hangs for dependency/factory live loops** — the dependency live loop's block-skip fast path decremented the active-task counter without a matching increment, underflowing it (it is unsigned) and making `rindexer`'s shutdown wait for in-flight work spin forever. Most visible with factory contracts whose discovery event is rare (e.g. a pool/vault factory): shutdown would report an enormous number of "active indexing tasks pending" and never exit.
- fix: **Upsert `subtract` operations now create rows at `-value`** — previously a row CREATED by a subtract operation (e.g. a token holder first seen as a *sender*) was inserted with the raw `+value`, corrupting the sign of aggregates built on subtract/decrement upserts. A fresh row now starts at `-value` (a subtraction from the implicit starting value of 0), so invariants like `SUM(balances) == credits - debits` hold exactly even when indexing starts mid-chain, and reorg reversal of such rows nets back to zero. Applies to both no-code custom tables and typed-project batch operations; `if:` conditions comparing the event value (e.g. `$value > 100`) still see the positive value. Tables built with subtract upserts before this fix may hold wrong-signed rows for entities first seen through a subtract — re-sync affected tables to correct them.

### Features
-------------------------------------------------

- feat: **`sync_together` — per-block atomic commits for live indexing** — no-code projects can define named groups of events (top-level `sync_together:` groups or a `sync_together: true` flag on a custom table) that advance block-by-block in lockstep during live indexing. For each block, every grouped event's raw rows, custom-table operations, reorg journal, and `last_synced_block` checkpoints commit in one Postgres transaction per block, per network — readers never see a block partially applied across the group, and the crash window where data lands without its checkpoint (double-index on restart) is closed. Postgres storage only (any ClickHouse storage is rejected), live-only, historical backfill unchanged. Behavior changes for grouped events: streams/chat/CSV dispatch **after** the block commit (at-most-once on crash between commit and dispatch; payloads are never published for uncommitted blocks and retries can't double-publish), and reorg-journal (`derived_op_log`) failures inside the block transaction now abort and retry the block instead of being logged and swallowed. `start_block` is optional for grouped members: without one, a member starts at the chain tip on first boot and — unlike ungrouped events — resumes from its checkpoint on restarts, so downtime never becomes a silent gap in the group. Factory-DEPLOYED contracts (children, e.g. factory-created vaults or pools) can be grouped: discovery stays on the eager factory pipeline and the group loop never processes a block past the factory's discovery checkpoint, keeping child address sets complete for every committed block (contracts used as a factory SOURCE still cannot be members). See the [sync_together documentation](/docs/start-building/yaml-config/sync-together).
