...
- feat: **Stream delivery modes** — new `delivery` option on stream configs: `instant` (default) delivers events immediately with reorg notifications for reconciliation; `finalized` buffers events until past the safe distance before publishing.
- feat: **`on_reorg` callback** — new optional callback registered per contract, fired after reorg recovery with the list of invalidated tx hashes. Allows user code to reconcile external side effects.
...
- feat: **Reorg metrics** — five new Prometheus metrics: `rindexer_reorg_handling_duration_seconds`, `rindexer_reorg_events_deleted_total`, `rindexer_reorg_events_reindexed_total`, `rindexer_reorg_detection_source_total`, `rindexer_reorg_cascade_total`.
