...
- feat: **Pruned RPC detection** — rindexer now detects when an RPC endpoint is pruned and cannot serve historical data from the configured `start_block`. On startup, each network's RPC is probed by fetching the start block and verifying transaction receipts are available. If the RPC appears pruned, indexing fails with a clear error message suggesting archive node usage or adjusting `start_block`. Use `--ignore-prune-warnings` CLI flag or `RINDEXER_IGNORE_PRUNE_WARNINGS=1` env var to bypass the check.
