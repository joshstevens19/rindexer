...
# 0.43.3-beta - 15th September 2026

github branch - https://github.com/joshstevens19/rindexer/tree/release/0.43.3

- linux binary - https://github.com/joshstevens19/rindexer/releases/download/v0.43.3/rindexer_linux-amd64.tar.gz
- mac apple silicon binary - https://github.com/joshstevens19/rindexer/releases/download/v0.43.3/rindexer_darwin-arm64.tar.gz
- mac apple intel binary - https://github.com/joshstevens19/rindexer/releases/download/v0.43.3/rindexer_darwin-amd64.tar.gz
- windows binary - https://github.com/joshstevens19/rindexer/releases/download/v0.43.3/rindexer_win32-amd64.zip

### Bug fixes
-------------------------------------------------
- fix: **Chain-wide `filter:` contracts with custom `tables:` no longer fail startup** with `contract '<Name>FilterFilter' not found in manifest` (regression in v0.43.2, regardless of `reorg_handling`). The reorg rollback-plan setup looked an event's contract up by its registry name, which carries the `Filter` suffix twice for filter contracts that declare tables (their manifest entry is renamed once while resolving column types and once more when the event registry is built) while the manifest is re-read from disk; the lookup now falls back to matching filter contracts with the suffixes stripped and refuses an ambiguous match.


# 0.43.1-beta - 0.43.2-beta - 13th September 2026

github branch - https://github.com/joshstevens19/rindexer/tree/release/0.43.1

- linux binary - https://github.com/joshstevens19/rindexer/releases/download/v0.43.1/rindexer_linux-amd64.tar.gz
- mac apple silicon binary - https://github.com/joshstevens19/rindexer/releases/download/v0.43.1/rindexer_darwin-arm64.tar.gz
- mac apple intel binary - https://github.com/joshstevens19/rindexer/releases/download/v0.43.1/rindexer_darwin-amd64.tar.gz
- windows binary - https://github.com/joshstevens19/rindexer/releases/download/v0.43.1/rindexer_win32-amd64.zip

### Bug fixes
-------------------------------------------------
- fix: **Atomic custom-table writes on the Postgres no-code path**: when Postgres is the sole raw-event sink, custom-table operations (upsert, insert, update, delete), their reorg journal rows, the raw event rows and the last-synced cursor now commit in one transaction; a crash, SIGTERM or deadlock mid-batch no longer leaves table effects (for example balance `add` upserts, or append-only insert rows) applied twice on retry. Multi-row upserts (no-code tables and the `create_batch_postgres_operation!` macro) order rows by their conflict key to avoid lock-order deadlocks within a statement. New metric `rindexer_atomic_batches_total{status,reason}`. The atomic path holds one pooled connection per in-flight batch from BEGIN to COMMIT: size `DATABASE_POOL_SIZE` at or above the number of concurrently indexing events when `tables:` are in use. `Tables::` log lines on this path describe statements staged in the batch transaction and repeat if the batch is retried; the `Failed query:` log line now always includes the SQL body; the `Shutdown - checkpointing block ... (mid-batch)` info line is no longer emitted (nothing is written while rows are resolved, and the `(last block written)` line covers the between-operation case); the atomic raw-insert COPY failure line now reads `Error writing binary data, aborting early: ...`; every Postgres error rindexer renders through its `PostgresError` type (setup, migrations, indexes, relationships, factory sync, table and raw-insert writes) now includes the server message and SQLSTATE on one line (for example `db error: ERROR: deadlock detected (40P01)`). ClickHouse/CSV-alongside, factory and streams/chat configs keep the previous at-least-once behavior.
