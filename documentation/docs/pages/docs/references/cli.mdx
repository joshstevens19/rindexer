...
:::info
The health monitoring server runs on port 8080 by default. You can configure it in your `rindexer.yaml` file using the `health_override_port` setting. See the [Running](/docs/start-building/running#health-monitoring) documentation for more details.
:::
```
