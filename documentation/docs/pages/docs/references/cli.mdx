...
:::info
On startup, rindexer checks if each network's RPC can serve historical data from the configured `start_block`. If the RPC appears to be pruned (unable to serve historical blocks), indexing will fail with a helpful error message. Use `--ignore-prune-warnings` to bypass this check, or set the `RINDEXER_IGNORE_PRUNE_WARNINGS=1` environment variable.
:::
