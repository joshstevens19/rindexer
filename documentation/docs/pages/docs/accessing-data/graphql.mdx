...
When you define `relationships`(TODO DOC) between events rindexer will automatically create relationships between the events
in the database and expose them on the `GraphQL` interface, this means you can query the relationships within a single query
avoiding having to have multiple queries to get the data you need.

Lets walk through an example imagine we were playing around with the `lens` data and we had a relationship between the
`QuoteCreated` `quoteParams.profileId` and the `ProfileCreated` `profileId` events.

You would define the relationship in the `rindexer.yaml` file like so:
    ...
    relationships: // [!code focus]
      - contract_name: LensHub // [!code focus]
        ...
        linked_to: // [!code focus]
          - contract_name: LensHub // [!code focus]
            event_name: ProfileCreated // [!code focus]
...
So in this example the `allQuoteCreateds` and `quoteCreated` queries will allow you to get the `ProfileCreated` event
...
query AllQuoteCreateds {
  allQuoteCreateds {
    nodes {
      ...
      quoteParamsReferrerProfileIds
      quoteParamsReferrerPubIds
      quoteParamsReferenceModuleData
      quoteParamsActionModules
      quoteParamsActionModulesInitDatas
      quoteParamsReferenceModule
      quoteParamsReferenceModuleInitData
      pubId
      ...
      blockHash
      network
      quoteParamsProfile { // [!code focus]
        nodeId // [!code focus]
        contractAddress // [!code focus]
        profileId // [!code focus]
        creator // [!code focus]
        to // [!code focus]
        timestamp // [!code focus]
        txHash // [!code focus]
