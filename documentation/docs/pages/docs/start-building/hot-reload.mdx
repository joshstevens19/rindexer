...
| [sync_together](/docs/start-building/yaml-config/sync-together) group or table flag changed | Restart (affected contracts) |
...
- **ABI files are not watched** — only `rindexer.yaml` is monitored. Edits to ABI JSON files do not trigger a reload; restart manually to pick them up
