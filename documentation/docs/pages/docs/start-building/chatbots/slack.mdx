...
5. Scroll down to the Bot token scopes header and add `chat:write`, `chat:write.customize`, `channels:read` and `files:write`.
These are the permissions the bot needs to write messages and send files into the Slack channels.
6. Finally, scroll all the way up and click on Install to workspace, and Allow on the following screen.
This should now show a screen like this, with the Bot User OAuth Token visible. Take note of this token,
