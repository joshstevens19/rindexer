...
| `rindexer_atomic_batches_total` | Counter | `status`, `reason` | Atomic no-code batches (custom-table operations, journal, raw rows and cursor in one transaction) by outcome |

**Operation types:** `query`, `insert`, `update`, `delete`, `batch_insert`, `batch_execute`

**Atomic batch labels:** `status` is `committed` or `rolled_back`; `reason` is `ok` on commit and otherwise `deadlock`, `cursor_missing` or `error`. A `rolled_back` batch is harmless (nothing was written and the batch is retried), so alert on the rate of `reason="deadlock"` rather than on rollbacks as such. Recorded only on the Postgres-only custom-tables path (see [Delivery Semantics](/docs/start-building/tables#delivery-semantics)).
...
# Atomic batch deadlock rate (each one is a rolled back, retried batch)
rate(rindexer_atomic_batches_total{reason="deadlock"}[5m])
```
