...
:::info
This field is optional and only available for `no-code` projects with postgres storage.
:::

Named groups of events that commit atomically per block, per network during live indexing. [See more details](/docs/start-building/yaml-config/sync-together).
