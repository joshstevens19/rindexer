...
```bash
CLICKHOUSE_URL="http://[host]:[port]"  # or https://[host]:[port] for hosted services
...
:::info
Both HTTP and HTTPS protocols are supported. For hosted ClickHouse services (ClickHouse Cloud, Aiven, etc.), use HTTPS with the appropriate port (typically 8443):
```bash
