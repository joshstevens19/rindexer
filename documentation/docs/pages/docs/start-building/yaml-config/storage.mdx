...
When indexing with postgres rindexer keeps the network and contract last seen block in the database, this is to ensure that if the indexer goes down it can pick up where it left off.
...
:::info
When you start up rindexer it will drop the indexes and resync the historic data then apply them again before indexing the live data.
