...
:::warning
ClickHouse storage is incompatible with [sync_together](/docs/start-building/yaml-config/sync-together) - ClickHouse
has no cross-table transactions. Any ClickHouse storage on the project (including Postgres + ClickHouse dual storage)
combined with `sync_together` is rejected at startup with a validation error.
:::
