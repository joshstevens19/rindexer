...
indexed fields are arrays so you can filter many values in the indexed fields.
...
:::info
This is optional, if you do not provide an start block it will index the data from now and then live index as new blocks
