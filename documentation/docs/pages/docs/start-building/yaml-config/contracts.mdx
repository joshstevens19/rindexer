...
The path to the factory-deployed contract address in the event inputs. If a factory deploys more than one contract in a single event, you can provide an array of inputs. Supports deep property access in case of complex event types: `pool.address`.
