...
atomically per block during live indexing - the raw event rows, all of the event's table operations, and the
`last_synced_block` checkpoint land in one Postgres transaction per block, per network.
