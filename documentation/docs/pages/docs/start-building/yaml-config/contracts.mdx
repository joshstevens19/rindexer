...
:::info
Factory-deployed contracts can also be [sync_together](/docs/start-building/yaml-config/sync-together) group
members for per-block atomic commits - the group is gated by the factory's discovery checkpoint, see
...
The events you wish to include for **raw event logging** - each event creates a row in its own table (e.g., `transfer`, `approval`).

:::info
This is optional. If you only want custom [tables](#tables) without raw event logging, you can omit this entirely.
If neither `include_events` nor `tables` is specified, all events in the ABI will be indexed.
:::
...
:::info
`dependency_events` is mutually exclusive with [sync_together](/docs/start-building/yaml-config/sync-together) per event -
both claim ownership of an event's live indexing loop, so an event cannot be in a dependency chain and a sync group
at the same time. This also applies to dependencies auto-derived from postgres [relationships](/docs/start-building/yaml-config/storage#relationships).
:::
...
:::tip[Recommended for No-Code Projects]
**Custom tables are the most powerful feature for no-code indexing.** Instead of just logging raw events,
you can maintain derived state like token balances, NFT ownership, counters, and cross-chain aggregations -
all without writing any Rust code.
:::

Custom tables let you define exactly what data you want to track and how events should update it.
When using `tables`, you don't need `include_events` - rindexer will automatically subscribe to
the events your tables reference.

**Quick example - Track ERC20 token balances:**
...
**Atomic per-block commits with `sync_together`:**

Next to the `global` and `cross_chain` flags, a table can set `sync_together: true` to commit its events
atomically per block during live indexing - the raw event rows, all of the events' table operations, and the
`last_synced_block` checkpoints land in one Postgres transaction per block, per network. Most useful for tables
fed by multiple events (e.g. WETH balances from `Deposit` + `Withdrawal` + `Transfer`), which then advance
block-by-block together instead of each event committing on its own loop:
...
See the full semantics and requirements in the [sync_together documentation](/docs/start-building/yaml-config/sync-together).

For complete documentation including column types, operations (`upsert`, `update`, `delete`),
set actions (`add`, `subtract`, `max`, `min`), condition expressions (`if`), `global` tables,
`cross_chain` aggregation, and more examples:

[**Custom Tables Documentation →**](/docs/start-building/tables)
