...
Phantom events give you ability to add your own custom events or edit existing events for ANY smart contract
this allows you to get any data you need from the contract.

"Phantom events are a gasless events, logged on an off-chain execution environment that mirrors mainnet state in realtime."

Have you ever needed more information from a contract but you either do not control the contract as its
decentralised or you did not want to add more gas for the users? This is where phantom events come in.
Everyone has different needs and different use cases for what data they want to get from the contract.
Phantom events powered by Shadow and Overlay are rindexer solutions to this problem.

rindexer abstracts away all of the complexity and has first party support for getting up and running with phantom events.

rindexer uses Etherscan APIs to download source code and ABIs for the contracts you want to index. The rindexer Etherscan
API key is a shared API key, this means you will get rate limited if you use this too much
or if many people use this at the same time.
You can add your own API key [here](/docs/start-building/yaml-config/global#etherscan_api_key) which we advise you do.
...
rindexer uses its CLI for everything and phantom events behaves the same way. Each rindexer project by default
does not have phantom events enabled you have to set them up for each project.

:::info
rindexer uses `forge` to clone and compile the contracts.
If you do not have `foundry` installed it will install it for you as you run the `init` command.
:::
...
You will be asked to pick your provider and add your API key if you have not added one already. It will save the API key
...
```bash
rindexer phantom clone <CONTRACT_NAME> <NETWORK_NAME>
```
...
```bash
rindexer phantom clone RocketPoolETH ethereum
```

This will create you a folder called `phantom` in the root of your rindexer project and also create a the network name
...
contract RocketTokenRETH is RocketBase, ERC20, RocketTokenRETHInterface {
    ...
    event TransferTime(address indexed from, uint256 time); // [!code focus]

    ...

    function _beforeTokenTransfer(address from, address, uint256) internal override {
        // emit your own event
        emit TransferTime(from, block.timestamp); // [!code focus]
...
contract RocketTokenRETH is RocketBase, ERC20, RocketTokenRETHInterface {
    ...
    event TokensBurned(address indexed from, uint256 amount, uint256 ethAmount, uint256 time);
    event TransferTime(address indexed from, uint256 time);
...
```bash
rindexer phantom compile <CONTRACT_NAME> <NETWORK_NAME>
```

So using the same yaml example as above you would run the following command:

```bash
rindexer phantom compile RocketPoolETH ethereum
```
...
Deploying your phantom contract is different to deploying a normal contract. rindexer will take care of the uploading
the new phantom contract and all the mappings for you in the `rindexer.yaml` file.

```bash
rindexer phantom deploy <CONTRACT_NAME> <NETWORK_NAME>
```

So using the same yaml example as above you would run the following command:

```bash
rindexer phantom deploy RocketPoolETH ethereum
```
...
That is you now have created phantom events with rindexer.