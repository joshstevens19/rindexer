...
:::tip[Want to track balances, counters, or aggregations?]
Instead of just logging raw events, you can use **Custom Tables** to maintain derived state.
For example, track token balances that automatically update with each Transfer event, or count
user actions, or aggregate data across multiple chains.
