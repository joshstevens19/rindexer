...
use rindexer::{
    event::callback_registry::TraceCallbackRegistry, start_rindexer, GraphqlOverrideSettings, IndexingDetails, StartDetails,
};
...
`manage_process_signals: true` and `disable_health_server: false` preserve the default standalone rindexer runtime behavior. Embedding hosts can set `manage_process_signals: false` when they own process shutdown handling, and `disable_health_server: true` when they expose health checks outside rindexer.
...
- `BlockIndexingCompleted { chain_id, block_number }` - emitted when all events on a given chain
have indexed up to at least `block_number`.
