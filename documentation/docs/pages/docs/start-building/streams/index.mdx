...
For events in a [sync_together](/docs/start-building/yaml-config/sync-together) group, stream messages are
dispatched **after** the block's Postgres transaction commits. Payloads are never published for blocks that fail
to commit and block retries cannot double-publish, but if the process crashes between commit and dispatch — or a
