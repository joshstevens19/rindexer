...
`sync_together` requires Postgres storage, no `end_block`, and no factory contracts (`start_block` is optional -
without one a member starts at the tip on first boot and resumes from its checkpoint on restarts) - see the
