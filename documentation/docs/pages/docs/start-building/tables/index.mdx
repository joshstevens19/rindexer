...
`sync_together` requires Postgres storage, an explicit `start_block` on every network, no `end_block`, and no
factory contracts - see the [full reference](/docs/start-building/yaml-config/sync-together) for all requirements
and trade-offs.
