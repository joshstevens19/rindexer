    ...
    ERC20FilterEventType::Transfer(
        TransferEvent::new(
            Arc::new(|results, context| {
                Box::pin(async move {
                    println!("transfer_handler2 {}", results.len());
...
async fn transfer_handler2(registry: &mut EventCallbackRegistry) {
    ERC20FilterEventType::Transfer(
        TransferEvent::handler(|results, context| async move {}, no_extensions()).await,
    )
    .register(registry);
}

pub async fn erc20_filter_handlers(registry: &mut EventCallbackRegistry) {
    transfer_handler2(registry).await;
}
