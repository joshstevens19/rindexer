...
fn main() {
    ...
    if blockclock_dir.exists() {
        fs::remove_dir_all(&blockclock_dir).expect("Failed to remove old resources directory");
    }

    fs::create_dir_all(&blockclock_dir).expect("Failed to create resources directory");
