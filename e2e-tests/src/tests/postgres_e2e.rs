        ...
        if let Some(r) = row {
            ...
            if count <= 0 {
                return Err(anyhow::anyhow!(
                    "Expected at least 1 row in simpleerc20_transfer, got {}",
                    count
                ));
            }
        } else {
            return Err(anyhow::anyhow!(
                "simpleerc20_transfer table not found or query returned no rows"
            ));
        }
...
fn postgres_live_exact_events_test(
    ...
    Box::pin(async move {
        ...
        let expected_recipients =
            vec![expected_address_for_counter(0), expected_address_for_counter(1)];
