...
fn demo_yaml_test(context: &mut TestContext) -> Pin<Box<dyn Future<Output = Result<()>> + '_>> {
    Box::pin(async move {
        ...
        let yaml_text =
            serde_yaml::to_string(&config).context("Failed to serialize demo config to YAML")?;
        std::fs::write(&target_yaml_path, yaml_text)
            .context("Failed to write updated YAML file")?;

        // Copy the SimpleERC20 ABI file
        let demo_abi_path =
            std::path::Path::new(env!("CARGO_MANIFEST_DIR")).join("abis/SimpleERC20.abi.json");
        ...
        std::fs::copy(&demo_abi_path, &target_abi_path).context("Failed to copy ABI file")?;
        ...
        rindexer.start_indexer().await.context("Failed to start Rindexer instance")?;
